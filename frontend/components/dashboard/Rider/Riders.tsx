...
import {
  formatDate,
  getNextPaymentDate,
} from "@/components/helperMethods/FormatDate";
...
const Riders = () => {
  ...
  const [mandateStatusUpdates, setMandateStatusUpdates] = useState<
    Record<string, string>
  >({});
  const [checkingStatus, setCheckingStatus] = useState<string | null>(null);
  const [cancellingMandate, setCancellingMandate] = useState<string | null>(
    null
  );
  ...
  const fetchRiders = async () => {
    try {
      ...
      const response = await apiClient.get(
        `${API_ENDPOINTS.RIDERS.LIST}?${params}`
      );
  ...
  const handleCheckStatus = async (riderId: string) => {
    try {
      ...
      const response = await apiClient.post(
        API_ENDPOINTS.RIDERS.CHECK_MANDATE(riderId)
      );

      if (response.data.success) {
        // Get the updated status from response (backend returns currentStatus)
        const updatedStatus =
          response.data.currentStatus || response.data.rider?.mandateStatus;

        // Refresh the riders list to get all updated data from the server
        await fetchRiders();

        toast.success(`Mandate status updated: ${updatedStatus}`);
      } else {
  ...
  const handleCancelMandate = async (riderId: string) => {
    if (
      !confirm(
        "Are you sure you want to cancel this mandate? This will stop all future automatic payments for this rider."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const response = await apiClient.post(
        API_ENDPOINTS.RIDERS.CANCEL_MANDATE(riderId)
      );
      ...
      if (response.data.success) {
        ...
        setRiders((prevRiders) =>
          prevRiders.map((r) =>
            r._id === riderId ? { ...r, mandateStatus: "cancelled" } : r
          )
        );
  ...
  const handleDeleteRider = async (riderId: string) => {
    ...
    try {
      ...
      const response = await apiClient.delete(
        API_ENDPOINTS.RIDERS.DELETE(riderId)
      );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "mandateStatusUpdate",
      handleMandateStatusUpdate as EventListener
    );
    return () => {
    ...
    const pollMandateStatus = async () => {
      try {
        ...
        for (const rider of pendingRiders) {
          try {
            const response = await apiClient.get(
              API_ENDPOINTS.RIDERS.MANDATE_STATUS(rider._id)
            );

            if (
              response.data.success &&
              response.data.mandateStatus !== rider.mandateStatus
            ) {
              ...
              window.dispatchEvent(
                new CustomEvent("mandateStatusUpdate", {
                  detail: {
            ...
            console.error(
              `Error checking status for rider ${rider.riderId}:`,
              error
            );
          }
        }
  ...
  const handleExportCSV = async () => {
    try {
      ...
      toast.dismiss(loadingToast);
      toast.success(
        `CSV exported successfully with ${allRiders.length} riders`
      );
    } catch (error: any) {
  ...
  const getMandateStatusBadge = (rider: Rider) => {
    const currentStatus =
      mandateStatusUpdates[rider.riderId] || rider.mandateStatus;
    const isUpdating =
      mandateStatusUpdates[rider.riderId] &&
      mandateStatusUpdates[rider.riderId] !== rider.mandateStatus;
    ...
    return (
      <div className="flex w-full h-full bg-[#F8F8F8] items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 lg:mt-4 text-black text-sm lg:text-base">
            Loading Riders...
          </p>
        </div>
      </div>
        ...
        <main className="flex-1 flex flex-col overflow-y-auto gap-4 lg:gap-6 p-3 lg:p-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex flex-col items-start justify-start gap-2">
              <h1 className="text-xl lg:text-2xl font-semibold text-black border-b-2 border-[#E51E25] pb-1">
                Riders
              </h1>
              <p className="text-sm lg:text-lg font-normal text-black">
                Manage bike rental Riders and their Payment mandates (Weekly)
              </p>
            </div>
          ...
          <div className="bg-white rounded-xl shadow p-3 lg:p-4">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-4 lg:mb-6">
              <div className="relative flex-1 w-full lg:max-w-md">
                ...
                />
                <svg
                  className="absolute left-2 lg:left-3 top-2.5 h-4 w-4 lg:h-5 lg:w-5 text-black"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <option value="active" className="py-2 px-3">
                        🟢 Active
                      </option>
                      <option value="failed" className="py-2 px-3">
                        🔴 Failed
                      </option>
                      <option value="cancelled" className="py-2 px-3">
                        ⚫ Cancelled
                      </option>
                      <option value="revoked" className="py-2 px-3">
                        ⚪ Revoked
                      </option>
                      <option value="pending" className="py-2 px-3">
                        ⚪ Pending
                      </option>
                    </select>
                    <div className="absolute inset-y-0 right-0 flex items-center pr-2 lg:pr-3 pointer-events-none">
                      <svg
                        className="h-3 w-3 lg:h-4 lg:w-4 text-black"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
              ...
              <table className="w-full border-collapse-separate min-w-[800px]">
                <thead className="bg-[#020202] border border-[#FEFDFF]">
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-2 lg:py-3 px-2 lg:px-4 font-normal border border-r border-[#FFFFFF] text-white rounded-tl-[12px] text-xs lg:text-sm">
                      Riders
                    </th>
                    <th className="text-left py-2 lg:py-3 px-2 lg:px-4 font-normal border border-r border-[#FFFFFF] text-white text-xs lg:text-sm">
                      Contact Info
                    </th>
                    <th className="text-center py-2 lg:py-3 px-2 lg:px-4 font-normal border border-r border-[#FFFFFF] text-white text-xs lg:text-sm">
                      Mandate Status
                    </th>
                    <th className="text-center py-2 lg:py-3 px-2 lg:px-4 font-normal border border-r border-[#FFFFFF] text-white text-xs lg:text-sm">
                      Amount (Weekly)
                    ...
                    <th className="text-center py-2 lg:py-3 px-2 lg:px-4 font-normal border border-r border-[#FFFFFF] text-white rounded-tr-[12px] text-xs lg:text-sm">
                      Actions
                    </th>
                  </tr>
                </thead>
                  ...
                  {riders.map((rider, index) => (
                    <tr key={rider._id} className="hover:bg-gray-50">
                      ...
                      <td className="text-center py-2 lg:py-3 px-2 lg:px-4 border border-[#0000001A]">
                        <span className="text-xs lg:text-sm text-gray-900">
                          {rider.nextDebitDate
                            ? formatDate(rider.nextDebitDate)
                            : "N/A"}
                        </span>
                      </td>
                      <td className="text-center py-2 lg:py-3 px-2 lg:px-4 border border-[#0000001A]">
